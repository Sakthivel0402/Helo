...
userdetails = f"""SELECT * FROM `userorder` WHERE `companystatus`='C TO A' AND id='{oid}'"""
...
print(f"""
<div class="d-flex justify-content-center mb-4"><h1>User Order Status</h1></div>
<a href="./index.py" <a href="./index.py" class="btn btn-danger me-5 float-end">Logout</a>
...
# Print progress bar steps
if delivered != 'delivered':
    print(f"""
        <li class="step0 {'active' if ordered == 'C TO A' or shipped == 'delivery' or ontheway == 'picked' or delivered == 'delivered' else ''}" id="step1">Ordered</li>
        <li class="step0 {'active' if shipped == 'delivery' or ontheway == 'picked' or delivered == 'delivered' else ''}" id="step2">Shipped</li>
        <li class="step0 {'active' if ontheway == 'picked' or delivered == 'delivered' else ''}" id="step3">Out for delivery</li>
        <li class="step0 {'active' if delivered == 'delivered' else ''}" id="step4">Delivered</li>""")
else:
    print(f"""
        <li class="step0 {'active2' if ordered == 'C TO A' or shipped == 'delivery' or ontheway == 'picked' or delivered == 'delivered' else ''}" id="step1">Ordered</li>
        <li class="step0 {'active2' if shipped == 'delivery' or ontheway == 'picked' or delivered == 'delivered' else ''}" id="step2">Shipped</li>
        <li class="step0 {'active2' if ontheway == 'picked' or delivered == 'delivered' else ''}" id="step3">Out for delivery</li>
...
if trash1 is not None:
    ...
    if reason and comments:
        cancel = f"UPDATE `userorder` SET `userstatus`='cancelled', `cancelreason`='{reason}', `cancelcomments`='{comments}', `cancelledtime`='{cancelledtime}' WHERE id={oid}"
        cur.execute(cancel)
        con.commit()
        print(f"<script>alert('Your order was cancelled for {reason}.'); location.href = 'menallproducts.py?id={id2}' </script>")
    else:
